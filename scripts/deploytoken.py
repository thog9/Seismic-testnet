...
# Constants
NETWORK_URL = "https://node-2.seismicdev.net/rpc"
CHAIN_ID = 5124
...
LANG = {
    'vi': {
        'title': '✨ TRIỂN KHAI TOKEN ERC20 - SEISMIC TESTNET ✨',
        'info': 'ℹ Thông tin',
        'found': 'Tìm thấy',
        'wallets': 'ví',
        ...
        'insufficient_balance': 'Số dư không đủ (cần ít nhất {required:.6f} ETH cho giao dịch)',
    ...
    'en': {
        'title': '✨ DEPLOY ERC20 TOKEN - SEISMIC TESTNET ✨',
        'info': 'ℹ Info',
        'found': 'Found',
        'wallets': 'wallets',
        ...
        'insufficient_balance': 'Insufficient balance (need at least {required:.6f} ETH for transaction)',
    ...
    for attempt in range(CONFIG['MAX_RETRIES']):
        try:
            ...
            if eth_balance < CONFIG['MINIMUM_BALANCE']:
                print(f"{Fore.RED}  ✖ {LANG[language]['insufficient_balance'].format(required=CONFIG['MINIMUM_BALANCE'])}: {eth_balance:.6f} ETH{Style.RESET_ALL}")
                return None
            ...
            if eth_balance < required_balance:
                print(f"{Fore.RED}  ✖ {LANG[language]['insufficient_balance'].format(required=required_balance)}: {eth_balance:.6f} ETH{Style.RESET_ALL}")
                return None
            ...
            if receipt is None:
                ...
                print(f"{Fore.YELLOW}    - {LANG[language]['balance']}: {w3.from_wei(w3.eth.get_balance(sender_address), 'ether'):.6f} ETH{Style.RESET_ALL}")
